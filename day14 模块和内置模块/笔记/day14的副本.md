...
在开发简单的程序时，使用一个py文件就可以搞定，如果程序比较庞大，需要些10w行代码，此时为了，代码结构清晰，将功能按照某种规则拆分到不同的py文件中，使用时再去导入即可。

<img src="assets/image-20210102110646946.png" alt="image-20210102110646946" style="zoom:50%;" />

另外，当其他项目也需要此项目的某些模块时，也可以直接把模块拿过去使用，增加重用性。
...
<img src="assets/image-20210102112657391.png" alt="image-20210102112657391" style="zoom:50%;" />
...
当定义好一个模块或包之后，如果想要使用其中定义的功能，必须要先导入，然后再能使用。例如：
...
- 一般项目相关的代码都会写在项目里（第三方模块自动安装在site-packages目录）。
- sys.path中的路径
  - 第一条，当前执行的脚本所在的路径。
  - 其他，Python内置的路径（第三方模块 & 内置模块）
- pycharm自动化会将项目目录加入到sys.path中。
- 一般执行的py文件，不在项目根目录时，需要用到自定义。
...
4. 如何生成随机数？
5. md5加密时加盐的目的是什么？
6. 写代码遍历文件夹下的所有png文件，并将录入记录到列表中。
